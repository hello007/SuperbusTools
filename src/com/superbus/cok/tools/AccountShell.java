...
{
	private static Logger logger = Logger.getLogger(AccountShell.class);
	...
	private static AccountShell main = new AccountShell();

	public static AccountShell getInstance()
	{
		return main;
	}
	...
	public static void main(String[] args)
	{
		try
		{
			AccountShell window = getInstance();
	...
	public void open(Shell parent, String serialNum)
	{
		Display display = Display.getDefault();
		logger.info("current num:" + serialNum);
		shell = new Shell(parent, ShellStyle.FixedSizePrimaryModal);
		curSerialNum = serialNum;
		init();
		shell.open();
		shell.layout();
	...
	{
		Display display = Display.getDefault();
		shell = new Shell(ShellStyle.FixedSize);
		init();
	...
	private void init()
	{
		...
		shell.setText(NameConstant.AccountName);
		final Image image = ImageUtil.getSuperbus();
		logger.info("获取图片:" + image);
		if (image != null)
		{
	...
	{
		// curSerialNum = PropertyMethod.getCurrentNum();
	...
	private void create差异性Composite()
	{
		...
		Label lblNewLabel = new Label(composite差异性, SWT.NONE);
		lblNewLabel.setAlignment(SWT.RIGHT);
		lblNewLabel.setForeground(SWTResourceManager.getColor(SWT.COLOR_RED));
		lblNewLabel.setBounds(336, 245, 262, 20);
		lblNewLabel.setText("修改后记得上传配置信息到服务器哟");
	...
	SelectionAdapter selectionAdapter = new SelectionAdapter()
	{
		...
		public void widgetSelected(SelectionEvent e)
		{
			...
			} else if (e.widget == button上传)
			{
				...
				try
				{
					ShowProcessDialog dialog = new ShowProcessDialog(shell, key);
					dialog.setTypeResult(Result.upload);
					...
					{
						MessageUtil.showErrorDialog(shell, "上传结果", "同步失败:"
								+ result);
						return;
					}
				} catch (Exception e1)
			...
			} else if (e.widget == button下载)
			{
				...
				{
					ShowProcessDialog dialog = new ShowProcessDialog(shell, key);
					dialog.setTypeResult(Result.download);
					String content = (String) dialog.open();
					...
					curSerialNum = key;
					PropertyMethod.saveSerialNum(curSerialNum);
